...
class Daisycon extends \Oara\Network
{
	...
	public function getTransactionList($merchantList = null, \DateTime $dStartDate = null, \DateTime $dEndDate = null)
	{
		...
		$password = $this->_credentials['password'];
		$id_site = $this->_credentials['idSite'];      // publisher id to retrieve (empty = all publishers)
        $mediaId = $this->_credentials['media_id'];      // media id to retrieve (empty = all media's)
		...
		foreach ($this->_publisherId as $publisherId) {
			...
			while (!$finish) {
				$url = "https://services.daisycon.com:443/publishers/$publisherId/transactions?media_id=$mediaId&page=$page&per_page=$pageSize&start=" . \urlencode($dStartDate->format("Y-m-d H:i:s")) . "&end=" . \urlencode($dEndDate->format("Y-m-d H:i:s"));
				// initialize curl resource
				$ch = curl_init();
				// set the http request authentication headers
				$headers = array('Authorization: Basic ' . base64_encode($user . ':' . $password));
